...
public class MessageRepositoryImpl implements MessageRepositoryCustom {
	...
	public List<Message> findByUserNum(Long userNum) {
		List<Message> resultList = entityManager.createQuery(
				"SELECT m FROM Message as m WHERE user1Num=?1 or user2Num=?2", Message.class)
				.setParameter(1, userNum)
				.setParameter(2, userNum)
				.getResultList();
		return resultList;
	}	
	...
	public List<Message> findByUserConv(Long userNum, Long youNum) {
		...
		if (userNum > youNum) {
			resultList = entityManager.createQuery(
					"SELECT m FROM Message as m WHERE (user1Num=?1 and user2Num=?2) "
					+ "and (msgStatus=?3 or msgStatus=?4)", Message.class)
					.setParameter(1, youNum)
					.setParameter(2, userNum)
					.setParameter(3, 3L)
					.setParameter(4, 1L)
					.getResultList();
	...
	public Integer changeMsgStatus(Long fromNum, Long toNum, Boolean flag) {
		String sql = "UPDATE Message m " +
				"SET msgStatus = msgStatus - :constant " +
				"WHERE user1Num = :user1Num and user2num= :user2Num "
				+ "and (msgStatus = 3 or msgStatus = :status)";
		int result = 0;
		if (flag) {			
			result = entityManager.createQuery(sql)
					.setParameter("constant", 2L)
					.setParameter("user1Num", fromNum)
					.setParameter("user2Num", toNum)
					.setParameter("status", 2L)
					.executeUpdate();
		} else {
			result = entityManager.createQuery(sql)
					.setParameter("constant", 1L)
					.setParameter("user1Num", toNum)
					.setParameter("user2Num", fromNum)
	...
	public List<Message> searchByUserNum(Long userNum) {
		List<Message> resultList = entityManager.createQuery(
				"SELECT m FROM Message as m WHERE user1Num = :userNum", Message.class)
				.setParameter(1, userNum)
				.getResultList();
		return resultList;
	}	
...
//		if (resultList == null) { return null;}
	...
	public List<Message> findLiveMsgs(Long mfn, Long mtn) {
		List<Message> resultList = entityManager.createQuery(
				"SELECT m FROM Message as m "
				+ "WHERE messageFromNum=?1 and messageToNum=?2 and senderStatus=1 or receiverStatus=1 ORDER BY sendDate DESC", Message.class)
				.setParameter(1, mfn)
                .setParameter(2, mtn)
				.getResultList();
        return resultList;
