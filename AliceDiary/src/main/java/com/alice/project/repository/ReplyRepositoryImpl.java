...
//	}	
	...
	@Override
	public List<Reply> findGroupByPostNum(Long postNum) {
		List<Reply> resultList = entityManager.createQuery(
				"SELECT NEW com.alice.project.domain.Reply(r.num, r.parentRepNum, r.content, r.repDate, r.heart, r.edit, r.post, r.member) "
				+ "FROM Reply AS r, Member AS m, "
				+ "Post AS p WHERE p.num=?1 "
				+ "GROUP BY r.num, r.parentRepNum, r.content, r.repDate, r.heart, r.edit, r.post, r.member", Reply.class)
				.setParameter(1, postNum)
				.getResultList();	
        return resultList;
	}
...
//	+ "GROUP BY r.reply_num, r.parent_rep_num, r.content, r.rep_date, r.member_num, r.heart, r.edit
