...
public class Post {
	...
	public Post(String title, LocalDateTime postDate, LocalDateTime updateDate, String content, Long viewCnt,
			PostType postType, Member member, Community community, List<Reply> replies, List<AttachedFile> files) {
		this.title = title;
	...
	// 게시물 객체 생성 메서드
	public static Post createPost(String title, LocalDateTime postDate, LocalDateTime updateDate, String content,
			Long viewCnt, PostType postType, Member member, Community community, List<Reply> replies,
			List<AttachedFile> files) {
		Post post = new Post(title, postDate, updateDate, content, viewCnt, postType, member, community, replies,
				files);
		return post;
	}
